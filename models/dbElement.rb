...
class DBElement
    ...
    def refresh_data data = nil
        ...
        mongo.collection.find( data ).each { |collection|
            ...
            clean_collection = self.class.bson_doc_to_hash collection
    ...
    def self.bson_doc_to_hash bson_doc
        ...
        bson_doc.each{ |key, val|
            if val.is_a? BSON::Document
                hash[key.to_sym] = self.bson_doc_to_hash val
            elsif val.is_a? Array
                hash[key.to_sym] = self.bson_doc_to_array val
            else
                hash[key.to_sym] = val
            end
        }
    ...
    def self.bson_doc_to_array bson_array
        ...
        bson_array.each{ |val|
            if val.is_a? BSON::Document
                array.push self.bson_doc_to_hash val
            elsif val.is_a? Array
                array.push self.bson_doc_to_array val
            else
                array.push val
            end
        }
